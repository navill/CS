...
CREATE TABLE `students` (
  ...
  KEY `fk_st_classID` (`classID`),
  CONSTRAINT `fk_st_classID` FOREIGN KEY (`classID`) REFERENCES `classes` (`classID`)
...
--
-- Temporary table structure for view `view_st_sb_join`
--

DROP TABLE IF EXISTS `view_st_sb_join`;
/*!50001 DROP VIEW IF EXISTS `view_st_sb_join`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `view_st_sb_join` (
...
--
-- Final view structure for view `view_st_sb_join`
--

/*!50001 DROP TABLE IF EXISTS `view_st_sb_join`*/;
/*!50001 DROP VIEW IF EXISTS `view_st_sb_join`*/;
...
/*!50001 VIEW `view_st_sb_join` AS select `ST`.`studentName` AS `studentName`,`ST`.`score` AS `score`,`SB`.`subjectName` AS `subjectName`,`SB`.`roomNum` AS `roomNum` from ((`students` `ST` join `student_subject` `SS` on(`ST`.`studentName` = `SS`.`studentName`)) join `subjects` `SB` on(`SS`.`subjectName` = `SB`.`subjectName`)) order by `ST`.`studentName` */;
...
-- Dump completed on 2019-04-04 23:16:59
